...
```bash
curl -X POST https://api.apilens.in/v1/traces/ \
...
from apilens.tracing import Tracer
import uuid
...
```javascript
import { apilens, Tracer } from '@apilens/sdk';
...
```bash
# Get trace by ID
curl "https://api.apilens.in/v1/traces/abc123def456" \
  -H "Authorization: Bearer YOUR_API_KEY"

# Search traces
curl "https://api.apilens.in/v1/traces/search/" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -G \
