...
description: 'Send and search structured logs with apilens'
---

# Collecting Logs

apilens provides powerful log aggregation and search capabilities for debugging and monitoring your applications.
...
```bash
curl -X POST https://api.apilens.in/v1/logs/ \
...
```bash
curl -X POST https://api.apilens.in/v1/logs/batch/ \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "logs": [
...
class apilensHandler(logging.Handler):
...
logger.addHandler(apilensHandler(api_key="YOUR_API_KEY"))
...
LOGGING = {
    ...
    'handlers': {
        'apilens': {
            'class': 'myapp.logging.apilensHandler',
            'api_key': os.environ.get('apilens_API_KEY'),
        },
    },
    'loggers': {
        'django': {
...
```javascript
import { apilens } from '@apilens/sdk';
import winston from 'winston';

const client = new apilens({ apiKey: 'YOUR_API_KEY' });

// Winston transport
class apilensTransport extends winston.Transport {
  log(info, callback) {
    client.logs.send({
      level: info.level,
...
const logger = winston.createLogger({
  transports: [new apilensTransport()]
});
...
```bash
# Search by message
curl "https://api.apilens.in/v1/logs/search/" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -G \
