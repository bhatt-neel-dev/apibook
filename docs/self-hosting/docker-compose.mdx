...
description: 'Deploy apilens with Docker Compose'
---

# Docker Compose Deployment

The easiest way to self-host apilens is using Docker Compose.
...
services:
  postgres:
    ...
    environment:
      POSTGRES_DB: ${POSTGRES_DB:-apilens}
      POSTGRES_USER: ${POSTGRES_USER:-apilens}
...
# Database shell
docker-compose exec postgres psql -U apilens
```
...
```bash
# PostgreSQL
docker-compose exec postgres pg_dump -U apilens apilens > backup.sql
...
```bash
# PostgreSQL
cat backup.sql | docker-compose exec -T postgres psql -U apilens apilens
