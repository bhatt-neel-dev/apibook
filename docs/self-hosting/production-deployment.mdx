...
description: 'Deploy apilens in production with SSL and security best practices'
---

# Production Deployment

This guide covers deploying apilens to a production server with SSL, security hardening, and monitoring.
...
curl -fsSL https://raw.githubusercontent.com/apilens-in/apilens/main/scripts/deploy-vm.sh -o deploy.sh
chmod +x deploy.sh
...
Create `/etc/systemd/system/apilens.service`:

```ini
[Unit]
Description=apilens Application
...
# Add this line
0 0 * * * certbot renew --quiet && docker compose -f /opt/apilens/docker-compose.prod.yml exec nginx nginx -s reload
```
